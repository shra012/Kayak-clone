...
const FlightsPage = () => {
  ...
  const loadFlights = async (overridePage, overrideFilters) => {
    ...
    // Validate that we have from and to airports before searching
    if (!activeFilters.from || !activeFilters.to) {
      console.warn('Cannot search flights without from/to airports');
      setError('Please select departure and destination airports');
      setLoading(false);
      return;
    }
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          {location.state?.search?.tripType !== 'multi-city' && (
            ...
            <button
              ...
              onClick={() => {
                console.log('Update button clicked with filters:', filters);
                if (!filters.from || !filters.to) {
                  toast.showError('Please enter both departure and destination airports');
                  return;
                }
                loadFlights(1);
              }}
                ...
                <>
                  ...
                  <div className="card bg-base-100 shadow-md">
                    <div className="card-body text-center py-12">
                      ...
                      <h3 className="text-xl font-semibold mb-2">No flights found for {filters.date}</h3>
                      <p className="text-base-content/60 mb-4">
                        {suggestedFlights.length > 0 
                          ? 'No flights available for the exact date. Check the suggestions above for nearby dates!'
                          : filters.from && filters.to 
                            ? `Try selecting a different date or adjusting your filters. Click the calendar to see available dates with pricing.`
                            : 'Please enter departure and destination airports, then select a date.'}
                      </p>
                      {alternativeDates.length > 0 && (
                        <div className="mb-4">
                          <p className="text-sm font-semibold mb-2">Try these dates with available flights:</p>
                          <div className="flex gap-2 justify-center flex-wrap">
                            {alternativeDates.slice(0, 3).map((alt) => (
                              <button
                                key={alt.date}
                                onClick={() => {
                                  const newFilters = { ...filters, date: alt.date };
                                  setFilters(newFilters);
                                  loadFlights(1, newFilters);
                                }}
                                className="btn btn-sm btn-outline"
                              >
                                {new Date(alt.date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} - ${alt.minPrice}
                              </button>
                            ))}
                          </div>
                        </div>
